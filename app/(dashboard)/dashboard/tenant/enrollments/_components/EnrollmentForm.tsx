...
export function EnrollmentForm({ classes, students }: { classes: ClassEntity[]; students: Student[] }) {
  ...
  return (
    <form action={action} className="space-y-4 rounded-xl border border-gray-200 bg-white p-6">
      ...
      <label className="block text-sm font-medium">Student<select name="student_id" required className="mt-1 min-h-11 w-full rounded-lg border border-gray-300 px-3"><option value="">Pilih student</option>{students.map((student) => <option key={student.id} value={student.id}>{student.full_name}</option>)}</select></label>
