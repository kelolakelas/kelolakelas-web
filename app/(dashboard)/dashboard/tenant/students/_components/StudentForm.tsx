...
export function StudentForm({ student }: { student?: Pick<Student, 'id' | 'first_name' | 'last_name' | 'date_of_birth'> }) {
  ...
  return <form action={action} className="space-y-4 rounded-xl border border-gray-200 bg-white p-5">
    ...
    <label className="block text-sm font-medium text-gray-800">Nama depan<input name="first_name" required defaultValue={student?.first_name} className="mt-1 min-h-11 w-full rounded-lg border border-gray-300 px-3" /></label>
    <label className="block text-sm font-medium text-gray-800">Nama belakang<input name="last_name" defaultValue={student?.last_name || ''} className="mt-1 min-h-11 w-full rounded-lg border border-gray-300 px-3" /></label>
