...
export default async function CatalogPage({ searchParams }: Props) {
  ...
  const tenantId = typeof params.tenant_id === 'string' ? params.tenant_id.trim() : '';
  const tenantName = tenantId
    ? result.data?.items.find(item => item.tenant_id === tenantId)?.tenant_name
    : undefined;
  const allClassesQuery = new URLSearchParams(query);
  allClassesQuery.delete('tenant_id');
  allClassesQuery.delete('page');
  const allClassesHref = allClassesQuery.size ? `/kelas?${allClassesQuery}` : '/kelas';
  const tenantFilterLabel = tenantName ? `Kelas dari ${tenantName}` : 'Filter tenant diterapkan';
